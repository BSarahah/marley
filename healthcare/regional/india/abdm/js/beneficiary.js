...
let verify_health_id = function (frm, recieved_abha_number = '') {
	let d = new frappe.ui.Dialog({
		...
		fields: [
			...
			{
				label: 'Mobile number to save beneficiary',
				fieldname: 'sb1',
		...
		primary_action(values) {
			...
			frappe.run_serially([
				() =>frappe.db.get_value('Beneficiary', {abha_number: d.get_value('healthid'), name: ['!=', frm.doc.name]	}, ['name', 'abha_card'])
					.then(r =>{
						if (r.message.name) {
							frappe.set_route("Form", "Beneficiary", r.message.name);
							if (r.message.abha_card) {
								frappe.throw({
									message: __(`{0}`,
									["<img src='"+ r.message.abha_card + "'>"]),
									title: __("Beneficiary already exist")
								});
							} else {
								frappe.throw({
									message: __(`{0}`,
									['<a href="/app/beneficiary/'+r.message.name+'">' + r.message.name + '</a>']),
									title: __("Beneficiary already exist")
								});
							}
						}
...
let create_abha = function (frm) {
	let d = new frappe.ui.Dialog({
		...
		fields: [
			...
			{
				fieldname: 'beneficiary_consent_attach',
				fieldtype: 'Attach',
				description: `Please attach beneficiary's signed consent for using
			...
			},
			{
				...
				fieldtype: 'Check',
				default: 0,
				description: `Check this to confirm that beneficiary has
		...
		primary_action(values) {
			if (!d.get_value('received_consent') && !d.get_value('beneficiary_consent_attach')) {
				frappe.throw({
					message: __(`Beneficiary Consent is required for ABHA creation`),
	...
	print_button.on('click', 'button', function() {
		frappe.db.get_value('Terms and Conditions', d.get_value('beneficiary_consent'), 'terms')
		.then(r => {
	...
	frappe.db.get_value('ABDM Settings', {
		...
		default: 1
	}, 'beneficiary_aadhaar_consent')
    .then(r => {
		if (r.message.beneficiary_aadhaar_consent) {
			d.set_values({
				'beneficiary_consent': r.message.beneficiary_aadhaar_consent
			});
		}
    })
	d.show();
...
let set_data_to_form = function(frm, scanned_data, dialog, d) {
	if (scanned_data) {
		...
		for (var k in scanned_data) {
			...
			if (d && d.get_value('beneficiary_consent_attach')) {
				frm.set_value('consent_for_aadhaar_use', d.get_value('beneficiary_consent_attach'))
			}
		}
						...
						frappe.call({
							...
							callback: function (data) {
								if (data.message['healthIdNumber']) {
									dialog.hide()
									frappe.run_serially([
										() =>frappe.db.get_value('Beneficiary', {abha_number: data.message['healthIdNumber'],
