...
def get(
	...
	for department in data:
		...
		department["total_appointments"] = frappe.db.count("Beneficiary Appointment", filters=filters)

		for entry in status:
			filters["status"] = entry
			department[frappe.scrub(entry)] = frappe.db.count("Beneficiary Appointment", filters=filters)
		filters.pop("status")
