...
def get_columns():
	return [
		...
		{
			"fieldname": "inbeneficiary_record",
			"fieldtype": "Link",
			"label": "Inbeneficiary Record",
			"options": "Inbeneficiary Record",
			"width": 200,
		},
	]
...
def get_data(filters):
	...
	data = frappe.db.sql(
		"""
		SELECT
			parent.beneficiary, parent.inbeneficiary_record, parent.practitioner,
			child.drug, child.drug_name, child.dosage, child.dosage_form,
			child.date, child.time, child.is_completed, child.name
		FROM `tabInbeneficiary Medication Order` parent
		INNER JOIN `tabInbeneficiary Medication Order Entry` child
	...
	data = get_inbeneficiary_details(data, filters.get("service_unit"))
...
def get_conditions(filters):
	...
	if filters.get("beneficiary"):
		conditions += " AND parent.beneficiary = %(beneficiary)s"
		values["beneficiary"] = filters.get("beneficiary")
...
def get_inbeneficiary_details(data, service_unit):
	service_unit_filtered_data = []

	for entry in data:
		entry["healthcare_service_unit"] = get_current_healthcare_service_unit(entry.inbeneficiary_record)
		if entry.is_completed:
			entry["inbeneficiary_medication_entry"] = get_inbeneficiary_medication_entry(entry.name)
...
def get_inbeneficiary_medication_entry(order_entry):
	return frappe.db.get_value(
		"Inbeneficiary Medication Entry Detail", {"against_imoe": order_entry}, "parent"
	)
...
def get_chart_data(data):
	...
	datasets.append(
		{
			"name": "Inbeneficiary Medication Order Status",
