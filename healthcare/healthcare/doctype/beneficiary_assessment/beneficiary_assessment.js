...
frappe.ui.form.on('Beneficiary Assessment', {
	refresh: function(frm) {
		...
		if (!frm.doc.__islocal) {
			frm.trigger('show_beneficiary_progress');
		}
	},
	...
	assessment_template: function(frm) {
		if (frm.doc.assessment_template) {
			frappe.call({
				...
				args: {
					doctype: 'Beneficiary Assessment Template',
	...
	show_beneficiary_progress: function(frm) {
		let bars = [];
		let message = '';
		let added_min = false;
...
frappe.ui.form.on('Beneficiary Assessment Sheet', {
