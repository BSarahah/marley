...
frappe.ui.form.on('Inbeneficiary Record', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		...
		if (!frm.doc.__islocal) {
			if (frm.doc.status == 'Admitted') {
				...
				frm.add_custom_button(__('Admit'), function() {
					admit_beneficiary_dialog(frm);
				} );
			} else if (frm.doc.status == 'Discharge Scheduled') {
				frm.add_custom_button(__('Discharge'), function() {
					discharge_beneficiary(frm);
				} );
			}
		}
		...
		frm.add_custom_button(__("Clinical Note"), function() {
			frappe.route_options = {
				...
				"reference_doc": "Inbeneficiary Record",
				"reference_name": frm.doc.name}
					frappe.new_doc("Clinical Note");
		},__('Create'));
	},
	btn_transfer: function(frm) {
		transfer_beneficiary_dialog(frm);
	}
});

let discharge_beneficiary = function(frm) {
	frappe.call({
		doc: frm.doc,
		method: 'discharge',
		...
		freeze: true,
		freeze_message: __('Processing Inbeneficiary Discharge')
	});
};

let admit_beneficiary_dialog = function(frm) {
	let dialog = new frappe.ui.Dialog({
		title: 'Admit Beneficiary',
		width: 100,
		fields: [
		...
		primary_action : function(){
			...
			frappe.call({
				...
				freeze: true,
				freeze_message: __('Processing Beneficiary Admission')
			});
			frm.refresh_fields();
			dialog.hide();
		}
...
let transfer_beneficiary_dialog = function(frm) {
	let dialog = new frappe.ui.Dialog({
	...
	dialog.fields_dict['leave_from'].get_query = function(){
		return {
			query : 'healthcare.healthcare.doctype.inbeneficiary_record.inbeneficiary_record.get_leave_from',
	...
	for (let inbeneficiary_occupancy in frm.doc.inbeneficiary_occupancies) {
		if (frm.doc.inbeneficiary_occupancies[inbeneficiary_occupancy].left != 1) {
			not_left_service_unit = frm.doc.inbeneficiary_occupancies[inbeneficiary_occupancy].service_unit;
		}
	}
	dialog.set_values({
...
let cancel_ip_order = function(frm) {
	...
	function(data) {
		frappe.call({
			...
			freeze: true,
			args: {
				inbeneficiary_record: frm.doc.name,
