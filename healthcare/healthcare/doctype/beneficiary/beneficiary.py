...
class Beneficiary(Document):
	...
	def after_insert(self):
		if frappe.db.get_single_value("Healthcare Settings", "collect_registration_fee"):
			frappe.db.set_value("Beneficiary", self.name, "status", "Disabled")
		else:
			send_registration_sms(self)
		self.reload()

	def on_update(self):
		if frappe.db.get_single_value("Healthcare Settings", "link_customer_to_beneficiary"):
			if self.customer:
				if self.flags.existing_customer or frappe.db.exists(
					{"doctype": "Beneficiary", "name": ["!=", self.name], "customer": self.customer}
				):
					self.update_beneficiary_based_on_existing_customer()
				else:
	...
	def autoname(self):
		beneficiary_name_by = frappe.db.get_single_value("Healthcare Settings", "beneficiary_name_by")
		if beneficiary_name_by == "Beneficiary Name":
			self.name = self.get_beneficiary_name()
		else:
	...
	def get_beneficiary_name(self):
		...
		if frappe.db.get_value("Beneficiary", name):
			count = frappe.db.sql(
				"""select ifnull(MAX(CAST(SUBSTRING_INDEX(name, ' ', -1) AS UNSIGNED)), 0) from tabBeneficiary
				 where name like %s""",
				"%{0} - %".format(name),
				as_list=1,
			)[0][0]
	...
	@frappe.whitelist()
	def invoice_beneficiary_registration(self):
		if frappe.db.get_single_value("Healthcare Settings", "registration_fee"):
			...
			frappe.db.set_value("Beneficiary", self.name, "status", "Active")
			send_registration_sms(self)
	...
	def set_contact(self):
		...
		if contact:
			...
			if self.customer:
				# customer contact exists, link beneficiary
	...
	def update_contact(self, contact):
		...
		contact.flags.skip_beneficiary_update = True
	...
	def update_linked_customer(self):
		...
		customer.customer_name = self.beneficiary_name
	...
	def update_beneficiary_based_on_existing_customer(self):
		customer = frappe.get_doc("Customer", self.customer)
		self.db_set(
			{
...
def create_customer(doc):
	customer = frappe.get_doc(
		{
			"doctype": "Customer",
			"customer_name": doc.beneficiary_name,
	...
	frappe.db.set_value("Beneficiary", doc.name, "customer", customer.name)
	frappe.msgprint(_("Customer {0} created and linked to Beneficiary").format(customer.name), alert=True)


def make_invoice(beneficiary, company):
...
@frappe.whitelist()
def get_beneficiary_detail(beneficiary):
	beneficiary_dict = frappe.db.sql("""select * from tabBeneficiary where name=%s""", (beneficiary), as_dict=1)
	if not beneficiary_dict:
		frappe.throw(_("Beneficiary not found"))
	vital_sign = frappe.db.sql(
		"""select * from `tabVital Signs` where beneficiary=%s
		order by signs_date desc limit 1""",
		(beneficiary),
		as_dict=1,
	)

	details = beneficiary_dict[0]
	if vital_sign:
		vital_sign[0].pop("inbeneficiary_record")
