...
frappe.ui.form.on('Beneficiary History Settings', {
	...
	field_selector: function(frm, doc, standard=1) {
		...
		frm.call({
			...
			callback: function(r) {
				if (r.message) {
					let doctype = 'Beneficiary History Custom Document Type';
					if (standard)
						doctype = 'Beneficiary History Standard Document Type';
	...
	get_date_field_for_dt: function(frm, row) {
		frm.call({
			...
			callback: function(data) {
				if (data.message) {
					frappe.model.set_value('Beneficiary History Custom Document Type',
...
frappe.ui.form.on('Beneficiary History Custom Document Type', {
...
frappe.ui.form.on('Beneficiary History Standard Document Type', {
	add_edit_fields: function(frm, cdt, cdn) {
		let row = locals[cdt][cdn];
		if (row.document_type) {
