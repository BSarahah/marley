...
from healthcare.healthcare.page.beneficiary_history.beneficiary_history import get_beneficiary_history_doctypes


class BeneficiaryHistorySettings(Document):
	def validate(self):
	...
	def validate_submittable_doctypes(self):
		for entry in self.custom_doctypes:
			if not cint(frappe.db.get_value("DocType", entry.document_type, "is_submittable")):
				...
				msg += _("Beneficiary Medical Record can only be created for submittable document types.")
				frappe.throw(msg)
...
def create_medical_record(doc, method=None):
	...
	if frappe.db.exists("Beneficiary Medical Record", {"reference_name": doc.name}):
		return

	subject = set_subject_field(doc)
	date_field = get_date_field(doc.doctype)
	medical_record = frappe.new_doc("Beneficiary Medical Record")
	medical_record.beneficiary = doc.beneficiary
...
def update_medical_record(doc, method=None):
	...
	medical_record_id = frappe.db.exists("Beneficiary Medical Record", {"reference_name": doc.name})

	if medical_record_id:
		subject = set_subject_field(doc)
		frappe.db.set_value("Beneficiary Medical Record", medical_record_id, "subject", subject)
	else:
...
def delete_medical_record(doc, method=None):
	...
	record = frappe.db.exists("Beneficiary Medical Record", {"reference_name": doc.name})
	if record:
		frappe.delete_doc("Beneficiary Medical Record", record, force=1)
...
def set_subject_field(doc):
	...
	subject = ""
	beneficiary_history_fields = get_beneficiary_history_fields(doc)

	for entry in beneficiary_history_fields:
...
def get_date_field(doctype):
	dt = get_beneficiary_history_config_dt(doctype)

	return frappe.db.get_value(dt, {"document_type": doctype}, "date_fieldname")


def get_beneficiary_history_fields(doc):
	dt = get_beneficiary_history_config_dt(doc.doctype)
	beneficiary_history_fields = frappe.db.get_value(
		dt, {"document_type": doc.doctype}, "selected_fields"
	)

	if beneficiary_history_fields:
		return json.loads(beneficiary_history_fields)
...
def get_beneficiary_history_config_dt(doctype):
	if frappe.db.get_value("DocType", doctype, "custom"):
		return "Beneficiary History Custom Document Type"
	else:
		return "Beneficiary History Standard Document Type"
...
def validate_medical_record_required(doc):
	...
	if doc.doctype not in get_beneficiary_history_doctypes():
		return False
