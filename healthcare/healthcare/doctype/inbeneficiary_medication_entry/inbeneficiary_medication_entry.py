...
class InbeneficiaryMedicationEntry(Document):
	def validate(self):
		self.validate_medication_orders()

	@frappe.whitelist()
	def get_medication_orders(self):
		# pull inbeneficiary medication orders based on selected filters
		...
		for data in orders:
			self.append(
				"medication_orders",
				{
					...
					"inbeneficiary_record": data.inbeneficiary_record,
	...
	def on_submit(self):
		...
		success_msg += _("Inbeneficiary Medication Orders updated successfully")
	...
	def validate_medication_orders(self):
		for entry in self.medication_orders:
			docstatus, is_completed = frappe.db.get_value(
				"Inbeneficiary Medication Order Entry", entry.against_imoe, ["docstatus", "is_completed"]
			)

			if docstatus == 2:
				frappe.throw(
					_(
						"Row {0}: Cannot create Inbeneficiary Medication Entry against cancelled Inbeneficiary Medication Order {1}"
	...
	def update_medication_orders(self, on_cancel=False):
		...
		frappe.db.sql(
			"""
			UPDATE `tabInbeneficiary Medication Order Entry`
		...
		for order, count in order_entry_map.items():
			medication_order = frappe.get_doc("Inbeneficiary Medication Order", order)
			completed_orders = flt(count)
			current_value = frappe.db.get_value("Inbeneficiary Medication Order", order, "completed_orders")
	...
	def make_stock_entry(self):
		...
		stock_entry.inbeneficiary_medication_entry = self.name
		...
		for entry in self.medication_orders:
			...
			# references
			se_child.beneficiary = entry.beneficiary
			se_child.inbeneficiary_medication_entry_child = entry.name

		stock_entry.submit()
		return stock_entry.name

	def cancel_stock_entries(self):
		stock_entries = frappe.get_all("Stock Entry", {"inbeneficiary_medication_entry": self.name})
...
def get_pending_medication_orders(entry):
	...
	data = frappe.db.sql(
		"""
		SELECT
			ip.inbeneficiary_record, ip.beneficiary, ip.beneficiary_name,
		...
		FROM
			`tabInbeneficiary Medication Order` ip
		INNER JOIN
			`tabInbeneficiary Medication Order Entry` entry
		ON
	...
	for doc in data:
		inbeneficiary_record = doc.inbeneficiary_record
		if inbeneficiary_record:
			doc["service_unit"] = get_current_healthcare_service_unit(inbeneficiary_record)
...
def get_filters(entry):
	...
	if entry.beneficiary:
		filters += " and ip.beneficiary = %(beneficiary)s"
		values["beneficiary"] = entry.beneficiary
...
def get_current_healthcare_service_unit(inbeneficiary_record):
	ip_record = frappe.get_doc("Inbeneficiary Record", inbeneficiary_record)
	if ip_record.status in ["Admitted", "Discharge Scheduled"] and ip_record.inbeneficiary_occupancies:
		return ip_record.inbeneficiary_occupancies[-1].service_unit
	return
...
def make_difference_stock_entry(docname):
	doc = frappe.get_doc("Inbeneficiary Medication Entry", docname)
