...
frappe.ui.form.on('Inbeneficiary Medication Entry', {
	refresh: function(frm) {
		...
		frm.add_custom_button(__('Make Stock Entry'), function() {
			frappe.call({
				method: 'healthcare.healthcare.doctype.inbeneficiary_medication_entry.inbeneficiary_medication_entry.make_difference_stock_entry',
				...
				callback: function(r) {
					if (r.message) {
						...
						frappe.msgprint({
							...
							message: __('All the drugs are available with sufficient qty to process this Inbeneficiary Medication Entry.'),
							indicator: 'green'
						});
					}
				}
