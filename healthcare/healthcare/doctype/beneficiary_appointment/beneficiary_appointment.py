...
class BeneficiaryAppointment(Document):
	def validate(self):
	...
	def set_title(self):
		if self.practitioner:
			self.title = _("{0} with {1}").format(
				self.beneficiary_name or self.beneficiary, self.practitioner_name or self.practitioner
			)
		else:
			self.title = _("{0} at {1}").format(
				self.beneficiary_name or self.beneficiary, self.get(frappe.scrub(self.appointment_for))
			)
	...
	def validate_overlaps(self):
		if self.appointment_based_on_check_in:
			if frappe.db.exists(
				...
				frappe.throw(_("Beneficiary already has an appointment booked for the same day!"), OverlapError)
			return
		...
		# all appointments for both beneficiary and practitioner overlapping the duration of this appointment
		overlapping_appointments = frappe.db.sql(
			"""
			SELECT
				name, practitioner, beneficiary, appointment_time, duration, service_unit
			FROM
				`tabBeneficiary Appointment`
			WHERE
				appointment_date=%(appointment_date)s AND name!=%(name)s AND status NOT IN ("Closed", "Cancelled") AND
				(practitioner=%(practitioner)s OR beneficiary=%(beneficiary)s) AND
			...
			if allow_overlap:
				service_unit_appointments = list(
					filter(
						...
						and appointment["beneficiary"] != self.beneficiary,
						overlapping_appointments,
					)
				)  # if same beneficiary already booked, it should be an overlap
	...
	def validate_based_on_appointments_for(self):
		if self.appointment_for:
			...
			# validate if beneficiary already has an appointment for the day
			...
			if booked_appointment:
				frappe.throw(
					_("Beneficiary already has an appointment {} booked for {} on {}").format(
						get_link_to_form("Beneficiary Appointment", booked_appointment),
	...
	def validate_service_unit(self):
		if self.inbeneficiary_record and self.service_unit:
			...
			is_inbeneficiary_occupancy_unit = frappe.db.get_value(
				"Healthcare Service Unit", self.service_unit, "inbeneficiary_occupancy"
			)
			service_unit = get_current_healthcare_service_unit(self.inbeneficiary_record)
			if is_inbeneficiary_occupancy_unit and service_unit != self.service_unit:
				msg = (
					_("Beneficiary {0} is not admitted in the service unit {1}").format(
						frappe.bold(self.beneficiary), frappe.bold(self.service_unit)
					)
					+ "<br>"
				)
				msg += _(
					"Appointment for service units with Inbeneficiary Occupancy can only be created against the unit where beneficiary has been admitted."
				)
	...
	def validate_customer_created(self):
		if frappe.db.get_single_value("Healthcare Settings", "show_payment_popup"):
			if not frappe.db.get_value("Beneficiary", self.beneficiary, "customer"):
				msg = _("Please set a Customer linked to the Beneficiary")
				msg += " <b><a href='/app/Form/Beneficiary/{0}'>{0}</a></b>".format(self.beneficiary)
	...
	def update_prescription_details(self):
		if self.procedure_prescription:
			...
			if self.procedure_template:
				...
				if comments:
					frappe.db.set_value("Beneficiary Appointment", self.name, "notes", comments)
	...
	def insert_calendar_event(self):
		...
		participants.append({"reference_doctype": "Beneficiary", "reference_docname": self.beneficiary})
	...
	def set_postition_in_queue(self):
		...
		if self.status == "Checked In" and not self.position_in_queue:
			appointment = frappe.qb.DocType("Beneficiary Appointment")
			position = (
...
@frappe.whitelist()
def check_payment_reqd(beneficiary):
	"""
	return True if beneficiary need to be invoiced when show_payment_popup enabled or have no fee validity
	...
	if show_payment_popup:
		if free_follow_ups:
			fee_validity = frappe.db.exists("Fee Validity", {"beneficiary": beneficiary, "status": "Active"})
			if fee_validity:
...
def invoice_appointment(appointment_name, discount_percentage=0, discount_amount=0):
	appointment_doc = frappe.get_doc("Beneficiary Appointment", appointment_name)
...
def create_sales_invoice(appointment_doc, discount_percentage=0, discount_amount=0):
	...
	sales_invoice.beneficiary = appointment_doc.beneficiary
	sales_invoice.customer = frappe.get_value("Beneficiary", appointment_doc.beneficiary, "customer")
	...
	if fee_validity:
		frappe.msgprint(
			_("{0} has fee validity till {1}").format(
				frappe.bold(appointment.beneficiary_name), format_date(fee_validity.valid_till)
			),
			alert=True,
		)


def check_is_new_beneficiary(beneficiary, name=None):
	filters = {"beneficiary": beneficiary, "status": ("!=", "Cancelled")}
	if name:
		filters["name"] = ("!=", name)

	has_previous_appointment = frappe.db.exists("Beneficiary Appointment", filters)
...
def get_appointment_item(appointment_doc, item):
	...
	item.amount = charge
	item.qty = 1
	item.reference_dt = "Beneficiary Appointment"
	item.reference_dn = appointment_doc.name
	return item


def cancel_appointment(appointment_id):
	appointment = frappe.get_doc("Beneficiary Appointment", appointment_id)
	if appointment.invoiced:
		...
		if frappe.db.get_single_value("Healthcare Settings", "enable_free_follow_ups"):
			fee_validity = frappe.db.get_value("Fee Validity", {"beneficiary_appointment": appointment.name})
			if fee_validity:
...
def check_sales_invoice_exists(appointment):
	sales_invoice = frappe.db.get_value(
		"Sales Invoice Item",
		{"reference_dt": "Beneficiary Appointment", "reference_dn": appointment.name},
		"parent",
	)
...
def update_status(appointment_id, status):
	frappe.db.set_value("Beneficiary Appointment", appointment_id, "status", status)
	...
	procedure_prescription = frappe.db.get_value(
		"Beneficiary Appointment", appointment_id, "procedure_prescription"
	)
...
def send_appointment_reminder():
	if frappe.db.get_single_value("Healthcare Settings", "send_appointment_reminder"):
		...
		for appointment in appointment_list:
			doc = frappe.get_doc("Beneficiary Appointment", appointment.name)
			message = frappe.db.get_single_value("Healthcare Settings", "appointment_reminder_msg")
			send_message(doc, message)
			frappe.db.set_value("Beneficiary Appointment", doc.name, "reminded", 1)


def send_message(doc, message):
	beneficiary_mobile = frappe.db.get_value("Beneficiary", doc.beneficiary, "mobile")
...
def get_events(start, end, filters=None):
	...
	conditions = get_event_conditions("Beneficiary Appointment", filters)

	data = frappe.db.sql(
		"""
		select
		`tabBeneficiary Appointment`.name, `tabBeneficiary Appointment`.beneficiary,
		`tabBeneficiary Appointment`.practitioner, `tabBeneficiary Appointment`.status,
		`tabBeneficiary Appointment`.duration,
		timestamp(`tabBeneficiary Appointment`.appointment_date, `tabBeneficiary Appointment`.appointment_time) as 'start',
		`tabAppointment Type`.color
		from
		`tabBeneficiary Appointment`
		left join `tabAppointment Type` on `tabBeneficiary Appointment`.appointment_type=`tabAppointment Type`.name
		where
		(`tabBeneficiary Appointment`.appointment_date between %(start)s and %(end)s)
		and `tabBeneficiary Appointment`.status != 'Cancelled' and `tabBeneficiary Appointment`.docstatus < 2 {conditions}""".format(
...
@frappe.whitelist()
def get_procedure_prescribed(beneficiary):
	return frappe.db.sql(
		"""
			...
			FROM
				`tabBeneficiary Encounter` ct, `tabProcedure Prescription` pp
			WHERE
				ct.beneficiary=%(beneficiary)s and pp.parent=ct.name and pp.appointment_booked=0
			ORDER BY
				ct.creation desc
		""",
		{"beneficiary": beneficiary},
...
@frappe.whitelist()
def get_prescribed_therapies(beneficiary):
	return frappe.db.sql(
		"""
			SELECT
			...
			FROM
				`tabBeneficiary Encounter` e, `tabTherapy Plan Detail` t
			WHERE
				e.beneficiary=%(beneficiary)s and t.parent=e.name
			ORDER BY
				e.creation desc
		""",
		{"beneficiary": beneficiary},
...
def update_appointment_status():
	...
	appointments = frappe.get_all(
		"Beneficiary Appointment", {"status": ("not in", ["Closed", "Cancelled"])}
	)

	for appointment in appointments:
		appointment_doc = frappe.get_doc("Beneficiary Appointment", appointment.name)
