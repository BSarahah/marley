...
frappe.ui.form.on('Beneficiary Appointment', {
	setup: function(frm) {
		frm.custom_make_buttons = {
			'Vital Signs': 'Vital Signs',
			'Beneficiary Encounter': 'Beneficiary Encounter'
		};
	},
	...
	refresh: function(frm) {
		...
		if (["Open", "Checked In"].includes(frm.doc.status) || (frm.doc.status == 'Scheduled' && !frm.doc.__islocal)) {
			...
			if (frm.doc.procedure_template) {
				...
				frm.add_custom_button(__('Beneficiary Encounter'), function() {
					frappe.model.open_mapped_doc({
						method: 'healthcare.healthcare.doctype.beneficiary_appointment.beneficiary_appointment.make_encounter',
						frm: frm,
					});
				}, __('Create'));
			}
		...
		frm.page.set_primary_action(__('Check Availability'), function() {
			if (!frm.doc.beneficiary) {
				frappe.msgprint({
					...
					message: __('Please select Beneficiary first'),
					indicator: 'red'
				});
			} else {
				check_and_set_availability(frm);
	...
	beneficiary: function(frm) {
		if (frm.doc.beneficiary) {
			...
			frm.set_value('inbeneficiary_record', '');
		}
	},
	...
	toggle_payment_fields: function(frm) {
		frappe.call({
			method: 'healthcare.healthcare.doctype.beneficiary_appointment.beneficiary_appointment.check_payment_reqd',
	...
	get_prescribed_therapies: function(frm) {
		if (frm.doc.beneficiary) {
			frappe.call({
				method: "healthcare.healthcare.doctype.beneficiary_appointment.beneficiary_appointment.get_prescribed_therapies",
				args: { beneficiary: frm.doc.beneficiary },
				callback: function(r) {
					if (r.message) {
						...
						frappe.msgprint({
							...
							message: __('There are no Therapies prescribed for Beneficiary {0}', [frm.doc.beneficiary.bold()]),
							indicator: 'blue'
						});
					}
				}
...
let check_and_set_availability = function(frm) {
						...
						frappe.call({
							...
							callback: (r) => {
								if (val && !r.message && !frm.doc.invoiced) {
									...
									frappe.call({
										method: "healthcare.healthcare.doctype.beneficiary_appointment.beneficiary_appointment.update_fee_validity",
										args: { "appointment": frm.doc }
									});
								}
							}
	...
	function show_slots(d, fd) {
		if (d.get_value('appointment_date') && d.get_value('practitioner')) {
			...
			frappe.call({
				method: 'healthcare.healthcare.doctype.beneficiary_appointment.beneficiary_appointment.get_availability_data',
				args: {
		...
		slot_details.forEach((slot_info) => {
			...
			if (fee_validity && fee_validity != 'Disabled') {
				slot_html += `
					...
					${__('Beneficiary has fee validity till')} <b>${moment(fee_validity.valid_till).format('DD-MM-YYYY')}</b>
					</span><br>`;
			} else if (fee_validity != 'Disabled') {
				slot_html += `
					<span style="color:red">
					${__('Beneficiary has no fee validity')}
					</span><br>`;
			}
...
let get_prescribed_procedure = function(frm) {
	if (frm.doc.beneficiary) {
		frappe.call({
			method: 'healthcare.healthcare.doctype.beneficiary_appointment.beneficiary_appointment.get_procedure_prescribed',
			args: { beneficiary: frm.doc.beneficiary },
			callback: function(r) {
				if (r.message && r.message.length) {
					...
					frappe.msgprint({
						title: __('Not Found'),
						message: __('No Prescribed Procedures found for the selected Beneficiary')
					});
				}
			}
		});
		...
		frappe.msgprint({
			title: __('Not Allowed'),
			message: __('Please select a Beneficiary first')
		});
	}
};
...
let show_procedure_templates = function(frm, result) {
	...
	if (!result) {
		let msg = __('There are no procedure prescribed for ') + frm.doc.beneficiary;
...
let update_status = function(frm, status) {
	...
	frappe.confirm(__('Are you sure you want to cancel this appointment?'),
		function() {
			frappe.call({
				method: 'healthcare.healthcare.doctype.beneficiary_appointment.beneficiary_appointment.update_status',
	...
	function show_payment_dialog(frm, fields) {
		let d = new frappe.ui.Dialog({
			...
			primary_action: async function(values) {
				...
				frappe.call({
					method: "healthcare.healthcare.doctype.beneficiary_appointment.beneficiary_appointment.invoice_appointment",
					args: {
		...
		d.set_values({
			"beneficiary": frm.doc.beneficiary_name,
