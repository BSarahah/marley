...
frappe.ui.form.on('Inbeneficiary Medication Order', {
	refresh: function(frm) {
		...
		frm.set_query('beneficiary', () => {
			return {
				filters: {
					'inbeneficiary_record': ['!=', ''],
					'inbeneficiary_status': 'Admitted'
				}
			};
		});
	},
	...
	show_get_from_encounter_button: function(frm) {
		frm.fields_dict['medication_orders'].grid.add_custom_button(__('Get From Encounter'), () => {
			if (!frm.doc.beneficiary_encounter) {
				frappe.throw(__("Please select a Beneficiary Encounter to get from"));
			}
			frm.call({
				doc: frm.doc,
				method: 'get_from_encounter',
				args: {
					encounter: frm.doc.beneficiary_encounter
				},
				freeze: true,
