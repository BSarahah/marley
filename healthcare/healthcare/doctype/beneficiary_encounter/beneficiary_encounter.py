...
class BeneficiaryEncounter(Document):
	def validate(self):
		self.set_title()
	...
	def on_cancel(self):
		...
		if self.inbeneficiary_record and self.drug_prescription:
	...
	def get_applicable_treatment_plans(encounter):
		...
		age = beneficiary.age
		if age:
			plan_filters["beneficiary_age_from"] = ["<=", age.years]
			plan_filters["beneficiary_age_to"] = [">=", age.years]
		...
		if diagnosis:
			...
			diagnosis = frappe.db.get_all("Beneficiary Encounter Diagnosis", filters=filters, fields="*")
		...
		if symptoms:
			...
			symptoms = frappe.db.get_all("Beneficiary Encounter Symptom", filters=filters, fields="*")
		...
		order = frappe.get_doc(
			{
				...
				"beneficiary": self.get("beneficiary"),
				"practitioner": self.practitioner,
				"source_doc": "Beneficiary Encounter",
				...
				"beneficiary_instruction": line_item.get("beneficiary_instruction"),
			}
		)
			...
			order.update(
				{
					...
					# "beneficiary_care_type": line_item.beneficiary_care_type
					# if line_item.beneficiary_care_type
					# else template_doc.get("beneficiary_care_type"),
				}
			)
	...
	def add_clinical_note(self, note, note_type=None):
		...
		clinical_note_doc.beneficiary = self.beneficiary
		clinical_note_doc.reference_doc = "Beneficiary Encounter"
	...
	@frappe.whitelist()
	def get_clinical_notes(self, beneficiary):
	...
	def get_encounter_details(self):
		medication_requests = []
		service_requests = []
		filters = {"beneficiary": self.beneficiary, "docstatus": 1}
			...
			if service_request.template_dt == "Lab Test Template":
				...
				if lab_test:
					subject = frappe.db.get_value(
						"Beneficiary Medical Record", {"reference_name": lab_test}, "subject"
					)
					if subject:
...
def make_ip_medication_order(source_name, target_doc=None):
	...
	doc = get_mapped_doc(
		...
		{
			"Beneficiary Encounter": {
				"doctype": "Inbeneficiary Medication Order",
				"field_map": {
					"name": "beneficiary_encounter",
					"beneficiary": "beneficiary",
					"beneficiary_name": "beneficiary_name",
					"beneficiary_age": "beneficiary_age",
					"inbeneficiary_record": "inbeneficiary_record",
...
def create_therapy_plan(encounter):
	if len(encounter.therapies):
		...
		doc.beneficiary = encounter.beneficiary
...
def delete_ip_medication_order(encounter):
	record = frappe.db.exists("Inbeneficiary Medication Order", {"beneficiary_encounter": encounter.name})
	if record:
		frappe.delete_doc("Inbeneficiary Medication Order", record, force=1)
...
def create_service_request(encounter):
	encounter_doc = frappe.get_doc("Beneficiary Encounter", encounter)
