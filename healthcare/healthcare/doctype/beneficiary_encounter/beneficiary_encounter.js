...
frappe.ui.form.on('Beneficiary Encounter', {
	onload: function(frm) {
		if (!frm.doc.__islocal && frm.doc.docstatus === 1 &&
			frm.doc.inbeneficiary_status == 'Admission Scheduled') {
				frappe.db.get_value('Inbeneficiary Record', frm.doc.inbeneficiary_record,
	...
	refresh: function(frm) {
		...
		if (!frm.doc.__islocal) {
			if (frm.doc.docstatus === 1) {
				if(!['Discharge Scheduled', 'Admission Scheduled', 'Admitted'].includes(frm.doc.inbeneficiary_status)) {
			...
			frm.add_custom_button(__('Beneficiary History'), function() {
				if (frm.doc.beneficiary) {
					frappe.route_options = {'beneficiary': frm.doc.beneficiary};
			...
			frm.add_custom_button(__("Clinical Note"), function() {
				frappe.route_options = {
					...
					"reference_doc": "Beneficiary Encounter",
			...
			if (frm.doc.drug_prescription && frm.doc.inbeneficiary_record && frm.doc.inbeneficiary_status === "Admitted") {
				frm.add_custom_button(__('Inbeneficiary Medication Order'), function() {
					frappe.model.open_mapped_doc({
						method: 'healthcare.healthcare.doctype.beneficiary_encounter.beneficiary_encounter.make_ip_medication_order',
						frm: frm
					});
				},__('Create'));
			}
		...
		frm.set_df_property('beneficiary', 'read_only', frm.doc.appointment ? 1 : 0);
		...
		if (frappe.meta.get_docfield('Drug Prescription', 'medication').in_list_view === 1) {
			frm.set_query('drug_code', 'drug_prescription', function(doc, cdt, cdn) {
				...
				if (row.medication) {
					return {
						query: 'healthcare.healthcare.doctype.beneficiary_encounter.beneficiary_encounter.get_medications_query',
	...
	beneficiary: function(frm) {
		frm.events.set_beneficiary_info(frm);
	},
	...
	set_appointment_fields: function(frm) {
		if (frm.doc.appointment) {
			...
			frm.set_value(values);
			frm.set_df_property('beneficiary', 'read_only', 0);
		}
	},

	set_beneficiary_info: async function(frm) {
		if (frm.doc.beneficiary) {
			let me = frm
			frappe.call({
				...
				callback: function(data) {
					...
					let values = {
						'beneficiary_age': age,
						'beneficiary_name':data.message.beneficiary_name,
						'beneficiary_sex': data.message.sex,
						'inbeneficiary_record': data.message.inbeneficiary_record,
						'inbeneficiary_status': data.message.inbeneficiary_status
					};
...
var schedule_inbeneficiary = function(frm) {
	...
	var dialog = new frappe.ui.Dialog({
		...
		primary_action : function() {
			...
			frappe.call({
				method: 'healthcare.healthcare.doctype.inbeneficiary_record.inbeneficiary_record.schedule_inbeneficiary',
				args: {
					args: args
				},
				callback: function(data) {
				...
				freeze: true,
				freeze_message: __('Scheduling Beneficiary Admission')
			});
			frm.refresh_fields();
			dialog.hide();
		}
...
var schedule_discharge = function(frm) {
	var dialog = new frappe.ui.Dialog ({
		...
		primary_action : function() {
			...
			frappe.call ({
				method: 'healthcare.healthcare.doctype.inbeneficiary_record.inbeneficiary_record.schedule_discharge',
				args: {args},
				callback: function(data) {
					if(!data.exc){
						frm.reload_doc();
				...
				freeze: true,
				freeze_message: 'Scheduling Inbeneficiary Discharge'
			});
			frm.refresh_fields();
			dialog.hide();
		}
...
let create_medical_record = function(frm) {
	if (!frm.doc.beneficiary) {
		frappe.throw(__('Please select beneficiary'));
	}
	frappe.route_options = {
		'beneficiary': frm.doc.beneficiary,
		'status': 'Open',
...
let cancel_ip_order = function(frm) {
	...
	function(data) {
		frappe.call({
			method: 'healthcare.healthcare.doctype.inbeneficiary_record.inbeneficiary_record.set_ip_order_cancelled',
			async: false,
			freeze: true,
			args: {
				inbeneficiary_record: frm.doc.inbeneficiary_record,
...
let create_service_request = function(frm) {
	frappe.call({
		method: "healthcare.healthcare.doctype.beneficiary_encounter.beneficiary_encounter.create_service_request",
		freeze: true,
		args: {
			encounter: frm.doc.name
		},
...
let create_medication_request = function(frm) {
	frappe.call({
		method: "healthcare.healthcare.doctype.beneficiary_encounter.beneficiary_encounter.create_medication_request",
		freeze: true,
		args: {
			encounter: frm.doc.name
		},
...
var show_clinical_notes = async function(frm) {
	if (frm.doc.docstatus == 0 && frm.doc.beneficiary) {
