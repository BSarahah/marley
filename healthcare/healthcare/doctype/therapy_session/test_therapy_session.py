...
class TestTherapySession(FrappeTestCase):
	def test_exercise_set_from_therapy_type(self):
		...
		session = create_therapy_session(plan.beneficiary, "Basic Rehab", plan.name)
...
def create_therapy_session(beneficiary, therapy_type, therapy_plan, duration=0, start_date=None):
	if not start_date:
		start_date = nowdate()
