...
@frappe.whitelist()
def get_therapy_sessions_count(beneficiary):
	total = frappe.db.count("Therapy Session", filters={"docstatus": 1, "beneficiary": beneficiary})

	month_start = datetime.today().replace(day=1)
	this_month = frappe.db.count(
		"Therapy Session", filters={"creation": [">", month_start], "docstatus": 1, "beneficiary": beneficiary}
	)
...
@frappe.whitelist()
def get_beneficiary_heatmap_data(beneficiary, date):
	return dict(
		frappe.db.sql(
			"""
		SELECT
		...
		ORDER BY communication_date asc""",
			{"date": date, "beneficiary": beneficiary},
		)
	)


@frappe.whitelist()
def get_therapy_sessions_distribution_data(beneficiary, field):
	if field == "therapy_type":
		result = frappe.db.get_all(
			"Therapy Session",
			filters={"beneficiary": beneficiary, "docstatus": 1},
			group_by=field,
			order_by=field,
			fields=[field, "count(*)"],
			as_list=True,
		...
		data = frappe.db.get_all(
			"Therapy Session", filters={"docstatus": 1, "beneficiary": beneficiary}, as_list=True
		)
...
@frappe.whitelist()
def get_therapy_progress_data(beneficiary, therapy_type, time_span):
...
@frappe.whitelist()
def get_beneficiary_assessment_data(beneficiary, assessment_template, time_span):
	date_range = get_date_range(time_span)
	query_values = {
		"from_date": date_range[0],
		"to_date": date_range[1],
...
@frappe.whitelist()
def get_therapy_assessment_correlation_data(beneficiary, assessment_template, time_span):
	date_range = get_date_range(time_span)
	query_values = {
		"from_date": date_range[0],
		"to_date": date_range[1],
	...
	result = frappe.db.sql(
		...
		FROM
			`tabBeneficiary Assessment` assessment INNER JOIN `tabTherapy Session` therapy
		ON
		...
		WHERE
			...
			assessment.beneficiary = %(beneficiary)s and
...
@frappe.whitelist()
def get_assessment_parameter_data(beneficiary, parameter, time_span):
	date_range = get_date_range(time_span)
	query_values = {
		"from_date": date_range[0],
		"to_date": date_range[1],
	...
	results = frappe.db.sql(
		...
		SELECT
			...
			template.scale_max
		FROM
			`tabBeneficiary Assessment Sheet` sheet
		INNER JOIN `tabBeneficiary Assessment` assessment
			ON sheet.parent = assessment.name
		INNER JOIN `tabBeneficiary Assessment Template` template
			ON template.name = assessment.assessment_template
		WHERE
			DATE(assessment.assessment_datetime) BETWEEN %(from_date)s AND %(to_date)s and
			assessment.docstatus = 1 and
			sheet.parameter = %(parameter)s and
			assessment.beneficiary = %(beneficiary)s
		ORDER BY
