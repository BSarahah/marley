frappe.provide('frappe.beneficiary_history');
frappe.pages['beneficiary_history'].on_page_load = function(wrapper) {
	frappe.ui.make_app_page({
		parent: wrapper,
		title: __('Beneficiary History')
	});

	let beneficiary_history = new BeneficiaryHistory(wrapper);
	$(wrapper).bind('show', ()=> {
		beneficiary_history.show();
	});
};
...
class BeneficiaryHistory {
	...
	show() {
		...
		beneficiary.df.change();
		beneficiary.refresh();

		if (frappe.route_options && !this.beneficiary_id) {
	...
	make_beneficiary_profile() {
		this.page.set_title(__('Beneficiary History'));
		this.main_section.empty().append(frappe.render_template('beneficiary_history'));
		this.setup_filters();
		this.setup_documents();
		this.show_beneficiary_info();
		this.setup_buttons();
		this.show_beneficiary_vital_charts('bp', 'mmHg', 'Blood Pressure');
	}
	...
	setup_filters() {
		...
		frappe.xcall(
			'healthcare.healthcare.page.beneficiary_history.beneficiary_history.get_beneficiary_history_doctypes'
		).then(document_types => {
			let doctype_filter = frappe.ui.form.make_control({
				...
				df: {
					...
					change: () => {
						me.start = 0;
						me.page.main.find('.beneficiary_documents_list').html('');
	...
	setup_documents(document_types="", selected_date_range="") {
		...
		let me = this;
		frappe.call({
			'method': 'healthcare.healthcare.page.beneficiary_history.beneficiary_history.get_feed',
			args: filters,
			callback: function(r) {
				let data = r.message;
				if (data.length) {
					...
					me.page.main.find('.beneficiary_documents_list').append(`
	...
	add_to_records(data) {
		...
		for (i=0; i<data.length; i++) {
			if (data[i].reference_doctype) {
				...
				details += `
					<div data-toggle='pill' class='beneficiary_doc_menu'
		...
		this.page.main.find('.beneficiary_documents_list').append(details);
		...
		if (data.length === 20) {
			...
			this.page.main.find(".beneficiary_documents_list").append(`
				<div class='text-muted' align='center'>
					<br><br>${__('No more records..')}<br><br>
				</div>`);
		}
	...
	show_beneficiary_info() {
		this.get_beneficiary_info().then(() => {
			$('.beneficiary-info').empty().append(frappe.render_template('beneficiary_history_sidebar', {
	...
	show_beneficiary_details() {
		...
		frappe.call({
			...
			callback: function(r) {
				...
				if (data.beneficiary_details) details += `<br><br><b> ${__('More info')} : </b> ${data.beneficiary_details.replace(/\n/g, ", ")}`;

				if (details) {
					details = `<div style='font-size:13px;' align='left'>` + details + `</div>`;
				}

				me.sidebar.find('.beneficiary-details').html(details);
			}
		});
	}
	...
	setup_buttons() {
		...
		this.page.main.on("click", ".btn-show-chart", function() {
			...
			me.show_beneficiary_vital_charts(btn_id, scale_unit, title);
		});
		...
		this.page.main.on('click', '.btn-more', function() {
			...
			if (me.page.main.find('.'+docname).parent().find('.document-html').attr('data-fetched') == '1') {
				...
				if (doctype && docname) {
					let exclude = ['beneficiary', 'beneficiary_name', 'beneficiary_sex', 'encounter_date', 'naming_series'];
					frappe.call({
	...
	show_beneficiary_vital_charts(btn_id, scale_unit, title) {
		let me = this;

		frappe.call({
			method: 'healthcare.healthcare.utils.get_beneficiary_vitals',
			args: {
				beneficiary: me.beneficiary_id
			},
			callback: function(r) {
				if (r.message) {
					...
					new frappe.Chart('.beneficiary_vital_charts', {
						data: {
							labels: labels,
							datasets: datasets
						},
					...
					me.page.main.find('.beneficiary_vital_charts').html('');
